...
public abstract class CFunc extends Problem {

	protected List< Problem > funcs_;
	...
	public CFunc(int dimensions, final int nofunc  ) { 
		
		super(dimensions);
		this.nofunc_ 		= nofunc;

		if (this.getDimensions() < 1 || nofunc_ < 1) 
	...
	private void initializeCFunc() {
		...
		O_  	= new double[nofunc_][this.getDimensions()];
		M_		= new double[nofunc_][this.getDimensions()][this.getDimensions()];

		weight_ = new double[nofunc_];
		fi_ 	= new double[nofunc_];
		z_  	= new double[this.getDimensions()];
		fmaxi_  = new double[nofunc_];
		tmpx_   = new double[this.getDimensions()];
	}
...
//				        System.out.println("");
	...
	void calculateWeights(final double [] x)
	{
		...
		for (int i=0; i<nofunc_; ++i) {
			...
			weight_[i] = Math.exp( -sum/(2.0 * this.getDimensions() * sigma_[i] * sigma_[i]) );
...
//			System.out.println("");
	...
	void initOptimaRandomly()
	{
		for (int i=0; i< nofunc_; ++i) {
			for (int j=0; j< this.getDimensions(); ++j) {
				...
				O_[i][j] = getLowerBound(j) + 
						(getUpperBound(j) - getLowerBound(j)) * Math.random();
	...
	{
		/* functions */
		double [] x5 = new double[this.getDimensions()];
		for (int i=0; i<this.getDimensions(); ++i) { x5[i] = 5 ; }
		int index=0;
		for( Problem f : funcs_ ) {
			transformToZNoshift(x5, index);
//			for (int i=0; i<z_.length; i++)
//				System.out.println("z_: "+this.z_[i]);
			fmaxi_[index] = f.evaluateSolution(this.z_); 
//			System.out.println("FMAXI: "+index+" : "+fmaxi_[index]);
	...
	double evaluateInner_(final double []x)
	{
	    ...
	    for( Problem f : funcs_ ) {
	        ...
	        fi_[index] = f.evaluateSolution(this.z_);
